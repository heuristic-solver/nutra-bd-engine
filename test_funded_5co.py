"""
test_funded_5co.py -- End-to-end BD intelligence run for 5 heavily-funded / VC-backed nutraceutical brands:
  1. Athletic Greens (AG1) -- Growth Equity / Series ($140M+)
  2. Ritual (Ritual Vitamins) -- Series A / Series B ($40M+)
  3. Seed Health (Microbiome Science) -- Series A ($40M+)
  4. HUM Nutrition (Clean Nutra & Beauty) -- Series A ($20M+)
  5. Care/of (Personalized Nutrition) -- Series A/B ($80M+)
"""
...
COMPANIES = [
    {"name": "Athletic Greens", "domain": "drinkag1.com",     "linkedin": "athleticgreens", "crunchbase_url": "https://www.crunchbase.com/organization/athletic-greens"},
    {"name": "Ritual",          "domain": "ritual.com",       "linkedin": "ritual-vitamins", "crunchbase_url": "https://www.crunchbase.com/organization/ritual"},
    {"name": "Seed Health",     "domain": "seed.com",         "linkedin": "seed-health",     "crunchbase_url": "https://www.crunchbase.com/organization/seed-health"},
    {"name": "HUM Nutrition",   "domain": "humnutrition.com", "linkedin": "hum-nutrition",   "crunchbase_url": "https://www.crunchbase.com/organization/hum-nutrition"},
    {"name": "Care/of",         "domain": "takecareof.com",   "linkedin": "takecareof",      "crunchbase_url": "https://www.crunchbase.com/organization/care-of"},
]

OUT_FILE = "bd_intelligence_funded_5co.csv"
SEP = "=" * 80
TIMEOUT = 120
...
def build_row(company, g, o, a, cb, career_data, traffic_data, serper_data, fda_data, scorecard):
    ...
    return {
        ...
        "cb_last_funding_type":         cb.get("last_funding_round_type", "Private / Self-Funded"),
        "cb_last_funding_amount":       cb.get("last_funding_amount", "N/A"),
        "cb_last_funding_date":         cb.get("last_funding_date", "N/A"),
        "cb_lead_investors":            cb.get("lead_investors", "N/A"),
        "cb_num_funding_rounds":        cb.get("num_funding_rounds", 0),
        "cb_num_acquisitions":          cb.get("num_acquisitions", 0),
        "cb_recent_acquisitions":       cb.get("recent_acquisitions", "None"),
        "cb_operating_status":          cb.get("operating_status", "Active"),
        ...
        "owler_estimated_revenue":   o.get("estimated_annual_revenue", ""),
...
def main():
    print(SEP)
    print("  BD INTELLIGENCE TEST RUN -- 5 FUNDED/VC-BACKED NUTRA BRANDS")
    ...
    growjo_raw = g_collector.fetch_companies(names, timeout_secs=TIMEOUT)
    growjo_idx = {r.get("company_name", "").strip().lower(): r for r in growjo_raw if r.get("company_name")}

    print("\n[Phase 2] Owler firmographic data...")
    o_collector = OwlerCollector(api_token=APIFY_TOKEN)
    owler_raw = o_collector.fetch_companies(names, timeout_secs=TIMEOUT)
    owler_idx = {r.get("company_name", "").strip().lower(): r for r in owler_raw if r.get("company_name")}

    print("\n[Phase 3] Apify LinkedIn snapshots...")
    a_collector = ApifyCollector(api_token=APIFY_TOKEN)
    li_urls = ["https://www.linkedin.com/company/" + c["linkedin"] + "/" for c in COMPANIES]
    apify_raw = a_collector.scrape_companies_batch(li_urls, timeout_secs=TIMEOUT)
    apify_idx = {r.get("company_name", "").strip().lower(): r for r in apify_raw if r.get("company_name")}

    print("\n[Phase 4] Apify Crunchbase funding & acquisitions...")
    cb_collector = CrunchbaseCollector(api_token=APIFY_TOKEN)
    cb_raw = cb_collector.fetch_companies_funding_batch(COMPANIES, timeout_secs=TIMEOUT)
    cb_idx = {r.get("company_name", "").strip().lower(): r for r in cb_raw if r.get("company_name")}

    print("\n[Phase 5] Per-company Synthesis (Career, Web Traffic, Serper, openFDA)...")
    ...
    for i, company in enumerate(COMPANIES, 1):
        ...
        print("    HC: " + str(row["primary_headcount"]) + " | Growth: " + str(row["employee_growth_pct"]) + " | Trajectory: " + str(row["hiring_trajectory"]))
        print("    Web: " + str(row["monthly_web_visits"]) + " (" + str(row["web_traffic_growth_pct"]) + ") | Career ATS: " + str(row["ats_platform"]) + " (" + str(row["active_job_openings_30d"]) + " jobs)")
        print("    CB Total Funding: " + str(row["total_funding"]) + " | Last Round: " + str(row["cb_last_funding_type"]) + " | Amount: " + str(row["cb_last_funding_amount"]))
        print("    Lead Investors: " + str(row["cb_lead_investors"]) + " | Acquisitions: " + str(row["cb_num_acquisitions"]))
